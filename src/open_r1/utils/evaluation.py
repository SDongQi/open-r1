...
核心逻辑:
1. 使用LightEval框架进行标准化的模型基准测试
2. 支持多种评估任务:
   - 数学能力评估(math_500, aime24, aime25)
   - 物理推理能力(gpqa)
   - 代码生成能力(lcb, lcb_v4)
3. 主要功能:
   - 任务注册系统: 统一管理各类评估任务
   - 作业调度: 通过Slurm在集群环境中并行执行评估
   - 资源优化: 根据模型大小自动决定GPU分配策略
   - 支持张量并行(Tensor Parallelism)处理大型模型
4. 工作流程:
   - 注册需要评估的任务
   - 根据模型配置确定硬件资源需求
   - 生成并提交Slurm作业脚本
...
此模块是Open-R1评估系统的核心，确保了不同模型在相同基准测试上的公平比较，
支持对SFT和GRPO训练产生的模型进行标准化评估，为模型性能提供客观度量。
"""
